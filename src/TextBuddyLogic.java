...
/**
 * TextBuddyLogic is the brain behind the TextBuddyProgram. This unit accepts the user command
 * and executes it. After executing the command it will return a feedback to TextBuddyProgram.
 * @author Jenny
 *
 */
public class TextBuddyLogic {
	...
	public Feedback addItem(String item) {
		if(item == EMPTY_STRING) {
			new Feedback(MSG_INVALID_ARG);
		} else {
			this.tempItemsStorage.add(++numItems + ". " + item);
		}
	...
	public Feedback displayItems() {
		if (this.numItems == 0) {
			return new Feedback(MSG_NO_ITEMS_TO_DISPLAY);
		} else {
			this.printItems();
			return new Feedback();
		}
	...
	/**
	 * @param deleteId The id of the item the user want to delete. 
	 ...
	 * @param itemDeleted the id to start renumbering from
	 * For instance after deleting the item 2
	 * 1. jopjpo => 1. jopjpo
	 * 3. popo => 2. popo
	 * 4. jpoo => 3. jpoo
	 */
	...
	/**
	 * @param item The word the user would like to search for.
	 */
	...
	/**
	 * Helper method for sortItems that strips each line of the id before sorting.
	 */
	...
	/**
	 * Helper method for sortItems to reassign new ids after the items have been sorted.
	 */
